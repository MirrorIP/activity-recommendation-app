...
public class CaptureTabGroupExperiences extends ExperiencesTab {
	
	protected enum Elements {
		VIEWEXPERIENCEBENEFIT("captureGroupExperiencesBenefit"),
		VIEWEXPERIENCEEFFORT("captureGroupExperiencesEffort"),
		VIEWEXPERIENCERATING("captureGroupExperiencesRating"),
		VIEWEXPERIENCESRATIOPANEL("captureGroupExperienceRatioPanel"),
		VIEWEXPERIENCESRATIO("captureGroupExperienceRatio"),
		VIEWEXPERIENCETABLE("captureGroupExperiencesExperiences");
	...
	public CaptureTabGroupExperiences(ARApp instance) {
		super(instance);
		initialize();
	}
	...
	public void showExperienceDetails(List<Experience> experiences, RecommendationObject recomm) {
		...
		if (experiences.size() > 0) {
			for (Experience exp:experiences) {
				if (exp.getSharingLevel() == 3 && !isModeratorOfRecommendation && !exp.getPublisher().contains(instance.getBareJID())) continue;
				final Experience ex = exp;

				if (exp.getSharingLevel() == 2 && !isModeratorOfRecommendation && !exp.getPublisher().contains(instance.getBareJID())) {
					ex.anonymizeExperience();
				}
